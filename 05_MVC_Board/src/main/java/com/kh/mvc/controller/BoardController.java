...
public class BoardController {
	...
	@PostMapping("/update")
	public String update(Board vo) throws IOException {	
		
		// 파일 업로드 기능 추가
		MultipartFile file = vo.getUploadFile();
		System.out.println(file);
		
		if(!file.isEmpty()) { // 업로드한 파일이 있는경우 !
			...
			if(vo.getUrl()!=null) {
				File delFile = new File(path+vo.getUrl().replace("/upload/",""));
				delFile.delete();
			}		
	...
	public String delete(int no) {
		Board vo = service.selectBoard(no);
		if(vo.getUrl()!=null) {
		File delFile = new File(path+vo.getUrl().replace("/upload/",""));
		delFile.delete();
	}		
	...
	public ModelAndView downloadFile(String filename) {
		HashMap map= new HashMap(); // spring에서 제공하는 view 로직이 hashmap으로 제공하기때문에 hashmap사용
		map.put("path", path);
		return new ModelAndView("downloadView",map);
	}

	public void fileUpload(Board vo) throws IOException {
		// 파일 업로드 기능 추가
		MultipartFile file = vo.getUploadFile();
